...
c = conn.cursor()
c.execute("DROP TABLE Steam_games")
#store your table in the database:

c.execute('''CREATE TABLE Steam_games 
...
c.executemany("INSERT INTO Steam_games(steam_appid, name, required_age, detailed_description, genres,release_date, review_score, total_positive, total_negative) VALUES (?,?,?,?,?,?,?,?,?)",
              ((df[3],df[2],df[4],df[6],df[23],df[26],df[31],df[33],df[34]) for df in stats))

for row in c.execute("SELECT * FROM Steam_games WHERE steam_appid IS 1198490"):
    print(row)
